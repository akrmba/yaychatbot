...
export class HubspotController {
  ...
  async callback(
    ...
    await this.hubspot.handleCallback(code, organizationId);
    const dashboardUrl = this.config.get<string>('DASHBOARD_URL', 'http://localhost:3001');
    res.redirect(`${dashboardUrl}/settings/integrations?connected=hubspot`);
  }
}
